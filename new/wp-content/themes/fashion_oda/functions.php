<?php
	...
	function customize_title($title) {
		...
		if(is_front_page()) {
			return "ファッション・服飾のプロを目指すなら｜織田ファッション専門学校";
		}
		elseif(is_tax()) {
		...
		elseif(is_post_type_archive("talk")) {
			return "ファッションを学ぶ学生へインタビュー！ | ".get_bloginfo();
		}
/* AIOで設定
...
*/
	...
	function customize_description($description) {
		...
		elseif(is_singular("talk")) {
			$description = '「'.get_the_title().'」をテーマに、ファッションとは切り離せない自分のポリシーについて、ファッション学生にインタビュー。東京都中野区の織田ファッション専門学校でスタイリスト、デザイナー、パタンナー、アパレル、ファッションビジネスのプロを目指そう！';
		}
	...
	function customize_ogp($value, $sns, $field) {
		if($field === 'sitename') {
			$value = "ファッション・服飾のプロを目指すなら｜織田ファッション専門学校";
		}
/*
...
<section id="oc" class="sec_oc2 mt70 is_anime fadein">
	<div class="wrap w1200">
		<div class="box_white">
			...
			<p class="lead">織田ファッション専門学校のオープンキャンパスでは<br class="sp">充実した授業体験を用意しています。</p>
				...
				if($val["class"] !== "prev" && $val["class"] !== "next") {
					...
					if($oc_arr[$date_txt]["design_technical"] || $oc_arr[$date_txt]["stylist_business"]) {
						...
						if($oc_arr[$date_txt]["design_technical"]) {
							$table .= '<li><a href="./oc"><img src="'.$tmp_dir.'/img/index/oc_icon_design_technical.png" alt=""></a></li>';
						}
						if($oc_arr[$date_txt]["stylist_business"]) {
							$table .= '<li><a href="./oc"><img src="'.$tmp_dir.'/img/index/oc_icon_stylist_business.png" alt=""></a></li>';
						}
						$table .= '</ul>';
					}
				}
...
<section class="sec_column mt70 is_anime fadein">
	<div class="wrap w1200">
		<div class="box_white">
			<h2 class="ttl_sec"><span class="en raleway">CAMPUS REPORT</span>キャンパスレポート</h2>
...
EOT;
	...
	function set_kv_image() {
		...
		if(is_front_page()) {
			...
			$html = <<<EOT
<img src="{$image_pc}" alt="ここから広がる私のミライ。" class="pc">
<img src="{$image_sp}" alt="ここから広がる私のミライ。" class="sp">
EOT;
		}
		else {
