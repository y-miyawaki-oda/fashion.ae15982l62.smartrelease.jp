...
<div class="box_white_under">
	...
	<section class="sec_contents">
		<h2 class="headline-2-1">様々な職種・フィールドで<br class="sp br_tab">活躍する卒業生たち</h2>
...
<?php foreach(get_terms("job", "hide_empty=1&orderby=menu_order&order=ASC") as $job): ?>
		<div id="<?php echo esc_attr($job->slug); ?>" class="mt80 sp-mt8 block-text-area">
			<h3 class="ttl_sec_sub"><span class="line"><?php echo esc_html($job->name); ?></span></h3>
	<?php
		$args = array(
			'posts_per_page' => -1,
			'post_type' => 'graduate',
	...
	<?php if($posts): ?>
		<?php foreach($posts as $post): setup_postdata($post); ?>
			...
			<p class="text-size24 bold tac"><?php the_field("company"); ?><br><?php the_title(); ?>/<?php the_field("job"); ?></p>
